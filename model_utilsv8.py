...
def get_yolo(img, model, confidence, color_pick_list, draw_thick):
    ...
    # Move image to same device as model if using CUDA
    device = next(model.parameters()).device
    if device.type == 'cuda':
        results = model.predict(source=img, conf=confidence, device=device)[0]
    else:
        results = model.predict(source=img, conf=confidence)[0]
    
    # Process predictions
    for r in results.boxes.data.tolist():
        xmin, ymin, xmax, ymax, conf, id = int(r[0]), int(r[1]), int(r[2]), int(r[3]), r[4], int(r[5])
    ...
    return img, current_no_class, detections
